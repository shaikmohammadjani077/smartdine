...
function BookTable() {
  ...
  useEffect(() => {
    ...
    const fetchOccupied = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(`http://localhost:5000/api/bookings/occupied?restaurantId=${restaurant._id}&date=${date}&time=${time}`, {
  ...
  useEffect(() => {
    ...
    const fetchTables = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/restaurants/${restaurant._id}/tables`);
        if (res.ok) {
          const data = await res.json();
          setTables(data || []);
        }
  ...
  const selectedTable = useMemo(() => {
    return availableTables.find((table) => table.tableNumber === selectedTableId) || null;
              ...
              <div className="table-legend">
                ...
                <div className="table-matrix">
                  {availableTables.map((table) => {
                    const isOccupied = occupiedTables.includes(table.tableNumber);
                    return (
                      <button
                        key={table._id}
                        type="button"
                      ...
                      >
                        <span className="t-id">{table.tableNumber}</span>
                        <span className="t-cap">👤 x {table.capacity}</span>
                        {isOccupied && <span className="occupied-tag">Occupied</span>}
                      </button>
                    );
                  })}
                </div>
