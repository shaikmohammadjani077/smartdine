...
export default function Dashboard() {
  ...
  const [editingTablesFor, setEditingTablesFor] = useState(null);
  ...
  const handleEditTables = async (restaurant) => {
    setEditingTablesFor(restaurant);
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://localhost:5000/api/admin/restaurants/${restaurant._id}/tables`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleAddTable = async () => {
    if (!editingTablesFor) return;
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://localhost:5000/api/admin/restaurants/${editingTablesFor._id}/tables`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const data = await res.json();
      if (res.ok) {
        setRestaurantTables((prev) => [...prev, data.table].sort((a, b) => a.tableNumber.localeCompare(b.tableNumber, undefined, { numeric: true })));
  ...
  const handleDeleteTable = async (tableId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://localhost:5000/api/admin/restaurants/${editingTablesFor._id}/tables/${tableId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        setRestaurantTables((prev) => prev.filter((t) => t._id !== tableId));
      }
    } catch (err) {
      console.error(err);
    }
        ...
        <div className="dash-container">
                ...
                restaurants.map((r) => (
                  <div key={r._id} className="wait-row">
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button className="assign-btn" style={{ background: "#444" }} onClick={() => handleEditTables(r)}>
                        Tables
                      </button>
          ...
          {editingTablesFor && (
            <div className="map-card" style={{ marginTop: "20px" }}>
              <div className="section-head">
                <h3>Tables for {editingTablesFor.name}</h3>
                <button className="assign-btn" style={{ background: "#666" }} onClick={() => setEditingTablesFor(null)}>Close</button>
              </div>
              
              <div className="admin-form" style={{ marginBottom: "20px" }}>
                <input
                  type="text"
                  placeholder="Table No (e.g. T11)"
                ...
                />
                <button className="walkin-btn" onClick={handleAddTable}>Add Table</button>
              </div>

              <div className="table-flex">
                {restaurantTables.map((t) => (
                  <div key={t._id} className="t-card available" style={{ cursor: "default" }}>
                    <div className="t-head">TABLE {t.tableNumber}</div>
                    <p>Capacity: {t.capacity}</p>
                    <button 
                      className="assign-btn" 
                      style={{ marginTop: "10px", width: "100%", background: "#ef4444" }} 
  ...
  return (
    <div className="dashboard-wrapper">
      <div className="dash-container">
        <div className="dash-top-bar">
          <div>
            ...
            <input 
              ...
              style={{ marginLeft: "15px", padding: "5px", borderRadius: "5px", border: "1px solid #ccc", background: "white", color: "black" }}
            />
          </div>
        ...
        {staffError && (
          <div style={{ background: "#fff3cd", border: "1px solid #ffc107", color: "#856404", padding: "12px", borderRadius: "5px", marginBottom: "15px" }}>
            ⚠️ <strong>Issue:</strong> {staffError}. Please contact your admin to assign a restaurant to your account.
          </div>
        )}
              ...
              waitlist.map((p) => (
                ...
                >
                  <div>
                    ...
                    <div className="cust-time">{p.time || "No time"} • {p.guests || 0} Guests</div>
                    {p.tableId && (
                      <div className="tag" style={{marginTop: "5px", fontSize: "10px", padding: "2px 8px", background: "rgba(34,197,94,0.1)", color: "#86efac", borderColor: "rgba(34,197,94,0.2)"}}>
                        Req: {p.tableId}
                      </div>
                    )}
                  </div>
