...
export const getOccupiedTables = async (req, res) => {
  ...
  if (!restaurantId || !date || !time) {
    return res.status(400).json({ message: "restaurantId, date, and time are required" });
  }
  ...
  const occupied = bookings.map((b) => b.tableId);
  res.json(occupied);
};

export const getWaitlistByRestaurant = async (req, res) => {
